...
from utils import *
from adabn import AdaBatchNorm2d, AdaBatchNorm1d
...
class Model(nn.Module):
    def __init__(self, params, adabn=True, train_model=True, prediction=False, glove=False, device="cuda"):
        ...
        self.glove_net = GLOVENet(d_e=params['d_e'], dp=params['dp_glove'], prediction=prediction) 
    ...
    def encode_emg(self, EMG, domain):
        return self.emg_net(EMG, domain)

    def encode_glove(self, GLOVE):
        return self.glove_net(GLOVE)

    def forward(self, EMG, GLOVE, domain):
        #ipdb.set_trace()
        if self.prediction:
            if self.glove:
                features = self.encode_glove(GLOVE)
            else:
                features = self.encode_emg(EMG, domain)
            features = features / features.norm(dim=-1,keepdim=True)
            return features
        else:
            emg_features = self.encode_emg(EMG, domain)
...
class EMGNet(nn.Module):
    def __init__(self, d_e, dp=.5, adabn=True, train=True, prediction=False, device="cuda"):
        ...
        self.conv_emg=nn.Sequential(
                ...
                nn.Conv2d(64,64,(1,1),padding=(1,1),bias=False),
                self.bn2d_func(64),
                nn.ReLU(),

                nn.AdaptiveAvgPool2d((1,EMG_DIM)),
                )
    ...
    def set_domain(self, domain):
        self.domain=domain
        self.apply(self._set_domain)

    def _set_domain(self, m):
        if isinstance(m, AdaBatchNorm2d) or isinstance(m, AdaBatchNorm1d):
            m.domain=self.domain

    def forward(self, EMG, domain):
        if self.adabn:
...
class GLOVENet(nn.Module):
    def __init__(self, d_e, dp=.5, train=True, prediction=False, device="cuda"):
        ...
        self.conv_glove=nn.Sequential(
                # conv -> bn -> relu
                nn.Flatten(),
                nn.Linear(GLOVE_DIM, GLOVE_DIM//4),
                nn.Linear(GLOVE_DIM//4,GLOVE_DIM),
                nn.Unflatten(1, (1,1,GLOVE_DIM)),
